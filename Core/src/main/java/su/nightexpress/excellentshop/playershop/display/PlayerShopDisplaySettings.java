...
public class PlayerShopDisplaySettings extends AbstractConfig implements DisplaySettings {
    ...
    private final ConfigProperty<Integer> itemChangeInterval = this.addProperty(ConfigTypes.INT,
        "Display.Item_Change_Interval",
        5,
        "Sets how often (in seconds) item displayed in the showcase will change.",
        "[*] Must be divisible by the Update_Interval value."
    );

    private final ConfigProperty<Integer> visibleDistance = this.addProperty(ConfigTypes.INT,
        "Display.Visible_Distance",
        10,
        "Sets shop display visibility distance."
    );

    private final ConfigProperty<Boolean> hologramEnabled = this.addProperty(ConfigTypes.BOOLEAN,
        "Display.Hologram.Enabled",
        true,
    ...
    private final ConfigProperty<Integer> hologramLineWidth = this.addProperty(ConfigTypes.INT,
        "Display.Hologram.LineWidth",
        200,
        "Maximum line width used to split lines.",
        "[Default is 200]"
    );

    private final ConfigProperty<Integer> hologramTextOpacity = this.addProperty(ConfigTypes.INT,
        "Display.Hologram.TextOpacity",
        -1,
    ...
    private final ConfigProperty<Boolean> hologramSeeThrough = this.addProperty(ConfigTypes.BOOLEAN,
        "Display.Hologram.SeeThrough",
        false,
        "Whether the text be visible through blocks.",
        "[Default is false]"
    );

    private final ConfigProperty<Boolean> hologramShadow = this.addProperty(ConfigTypes.BOOLEAN,
        "Display.Hologram.Shadow",
        true,
        "Whether the text is displayed with shadow.",
        "[Default is true]"
    );

    private final ConfigProperty<int[]> hologramBackgroundColor = this.addProperty(ConfigTypes.INT_ARRAY,
        "Display.Hologram.BackgroundColor",
        new int[]{64, 0, 0, 0},
        "The background color, arranged by [A,R,G,B]. Where: A = Alpha (opacity), R = Red, G = Green, B = Blue.",
        "[Default is 64,0,0,0]"
    );

    private final ConfigProperty<List<String>> hologramAdminShop = this.addProperty(ConfigTypes.STRING_LIST,
        "Display.Hologram.Content.AdminShop",
        getDefaultShopHologram(),
        "Hologram text for Admin Shops."
    );

    private final ConfigProperty<List<String>> hologramPlayerShop = this.addProperty(ConfigTypes.STRING_LIST,
        "Display.Hologram.Content.PlayerShop",
        getDefaultShopHologram(),
        "Hologram text for Player Shops."
    );

    private final ConfigProperty<List<String>> hologramRentable = this.addProperty(ConfigTypes.STRING_LIST,
        "Display.Hologram.Content.Rent",
        Lists.newList(
    ...
    private final ConfigProperty<List<String>> hologramAbsent = this.addProperty(ConfigTypes.STRING_LIST,
        "Display.Hologram.Content.Empty",
        Lists.newList(
    ...
    private static List<String> getDefaultShopHologram() {
        ...
        String stockText = gray.wrap("Stock: ") + white.wrap(ShopPlaceholders.PRODUCT_STOCK) + " " + gray.wrap("(" +
            white.wrap("+" + ShopPlaceholders.PRODUCT_SPACE));
