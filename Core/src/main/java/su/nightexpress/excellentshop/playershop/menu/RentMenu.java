...
public class RentMenu extends LinkedMenu<ShopPlugin, ChestShop> implements ConfigBased {
    ...
    public RentMenu(@NonNull ShopPlugin plugin, @NonNull ChestShopModule module) {
    ...
    protected void onItemPrepare(@NonNull MenuViewer viewer, @NonNull MenuItem menuItem, @NonNull NightItem item) {
        ...
        item.replace(replacer -> replacer.with(shop.placeholders()).andThen(shop.getRentSettings()
            .replacePlaceholder()));
    }
    ...
    private void handleToggle(@NonNull MenuViewer viewer, boolean state) {
        ChestShop shop = this.getLink(viewer);
    ...
    private void handleDuration(@NonNull MenuViewer viewer) {
        ChestShop shop = this.getLink(viewer);
    ...
    private void handlePrice(@NonNull MenuViewer viewer) {
        ChestShop shop = this.getLink(viewer);
    ...
    private void handleCancel(@NonNull MenuViewer viewer) {
        Player player = viewer.getPlayer();
        ChestShop shop = this.getLink(player);
    ...
    public void loadConfiguration(@NonNull FileConfig config, @NonNull MenuLoader loader) {
        loader.addDefaultItem(NightItem.fromType(Material.LIME_DYE)
            ...
            .setSlots(11)
            .setPriority(10)
            .setHandler(new ItemHandler("rent_off", (viewer, event) -> this.handleToggle(viewer, false), ItemOptions
                .builder()
        ...
        loader.addDefaultItem(NightItem.fromType(Material.GRAY_DYE)
            ...
            .setSlots(11)
            .setPriority(10)
            .setHandler(new ItemHandler("rent_on", (viewer, event) -> this.handleToggle(viewer, true), ItemOptions
                .builder()
                .setVisibilityPolicy(viewer -> !this.getLink(viewer).getRentSettings().isEnabled())
                .build()
            )));
        ...
        loader.addDefaultItem(NightItem.fromType(Material.EMERALD)
            ...
            .setSlots(16)
            .setPriority(10)
            .setHandler(new ItemHandler("rent_currency", (viewer, event) -> this.handleCurrency(viewer), ItemOptions
                .builder()
        ...
        loader.addDefaultItem(NightItem.fromType(Material.BARRIER)
            ...
            .setSlots(35)
            .setPriority(10)
            .setHandler(new ItemHandler("rent_cancel", (viewer, event) -> this.handleCancel(viewer), ItemOptions
                .builder()
                .setVisibilityPolicy(viewer -> this.getLink(viewer).isRented())
                .build()
            )));

        loader.addDefaultItem(MenuItem.buildReturn(this, 31, (viewer, event) -> this.handleReturn(viewer)).setPriority(
            10));
