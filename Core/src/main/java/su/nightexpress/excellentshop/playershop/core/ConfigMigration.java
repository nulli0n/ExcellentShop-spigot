...
public class ConfigMigration {
    ...
    public static void migrateHologramSettings(@NonNull FileConfig config) {
        ...
        textRent.replaceAll(str -> str
            .replace(ShopPlaceholders.GENERIC_TIME, ShopPlaceholders.CHEST_SHOP_RENT_EXPIRES_IN)
            .replace(ShopPlaceholders.GENERIC_PRICE, ShopPlaceholders.CHEST_SHOP_RENT_PRICE)
        );

        Replacer replacer = Replacer.create()
            .replace("%product_price_buy%", ShopPlaceholders.PRODUCT_PRICE_FORMATTED.apply(TradeType.BUY))
            .replace("%product_price_sell%", ShopPlaceholders.PRODUCT_PRICE_FORMATTED.apply(TradeType.SELL))
            .replace("%product_name%", ShopPlaceholders.PRODUCT_PREVIEW_NAME)
            .replace("%product_stock%", () -> ShopPlaceholders.PRODUCT_STOCK);
    ...
    public static void migrateShowcaseCatalog(@NonNull FileConfig config) {
        if (config.contains("Display.PlayerCustomization.Showcases")) {
            config.getSection("Display.PlayerCustomization.Showcases").forEach(sId -> {
                ...
                config.set("Display.Showcase.Catalog." + sId, new Showcase(sId, ShopPlaceholders.GENERIC_TYPE, item));
            });
            config.remove("Display.PlayerCustomization");
        }
    }

    public static void migrateShopBlocks(@NonNull FileConfig from, @NonNull FileConfig target) {
        ...
        if (from.contains("Display.Showcase") && !from.contains("Display.Showcase.Catalog")) {
            from.getSection("Display.Showcase").forEach(sId -> {
                ...
                oldContainerShowcases.put(sId.toLowerCase(), new Showcase(sId, ShopPlaceholders.GENERIC_TYPE, item));
            });
        ...
        oldContainers.forEach(type -> {
            ...
            NightItem item = oldContainerItems.getOrDefault(type.toLowerCase(), ChestUtils.getDefaultShopItem(
                material));
            Showcase showcase = oldContainerShowcases.getOrDefault(type.toLowerCase(),
                oldContainerShowcases.getOrDefault(ShopPlaceholders.DEFAULT, new Showcase(BukkitThing.getValue(
                    material), ShopPlaceholders.GENERIC_TYPE, NightItem.fromType(Material.GLASS))));
