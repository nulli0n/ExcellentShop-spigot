...
public class ShopView extends AbstractObjectMenu<ChestShop> {
    ...
    protected String getRawTitle(@NonNull ViewerContext context) {
        return PlaceholderContext.builder().with(this.getObject(context).placeholders()).build().apply(
            super.getRawTitle(context));
    }
    ...
    protected void onLoad(@NonNull FileConfig config) {
        this.productSlotsByCount = new TreeMap<>(config.get(ConfigTypes.forMap(Numbers::getIntegerAbs, String::valueOf,
            ConfigTypes.INT_ARRAY), "Product.SlotsByCount",
            Map.of(
                1, new int[]{2},
                2, new int[]{1, 3},
                3, new int[]{1, 2, 3},
