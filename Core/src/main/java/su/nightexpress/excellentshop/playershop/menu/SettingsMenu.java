...
public class SettingsMenu extends LinkedMenu<ShopPlugin, ChestShop> implements ConfigBased {
    ...
    public SettingsMenu(@NonNull ShopPlugin plugin, @NonNull ChestShopModule module) {
    ...
    protected String getTitle(@NonNull MenuViewer viewer) {
        return PlaceholderContext.builder().with(this.getLink(viewer).placeholders()).build().apply(this.title);
    }
    ...
    private void handleName(@NonNull MenuViewer viewer, @NonNull InventoryClickEvent event) {
        Player player = viewer.getPlayer();
    ...
    private void handleProducts(@NonNull MenuViewer viewer) {
        this.runNextTick(() -> this.module.openProductsMenu(viewer.getPlayer(), this.getLink(viewer)));
    }

    private void handleShops(@NonNull MenuViewer viewer) {
        this.runNextTick(() -> this.module.browseShopsByShop(viewer.getPlayer(), this.getLink(viewer)));
    }

    private void handleBank(@NonNull MenuViewer viewer) {
        this.runNextTick(() -> {
            Bank bank = module.getEffectiveBank(this.getLink(viewer));
            if (bank == null) return;

            module.openBankManagementDialog(viewer.getPlayer(), bank, null);
        });
    }

    private void handleDisplay(@NonNull MenuViewer viewer) {
        this.runNextTick(() -> this.module.openShowcaseMenu(viewer.getPlayer(), this.getLink(viewer)));
    }

    private void handleRentSettings(@NonNull MenuViewer viewer) {
        this.runNextTick(() -> this.module.openRentSettings(viewer.getPlayer(), this.getLink(viewer)));
    }

    private void handleTrustedPlayers(@NonNull MenuViewer viewer) {
        this.runNextTick(() -> this.module.openTrustedPlayers(viewer.getPlayer(), this.getLink(viewer)));
    }

    private void handleRentExtend(@NonNull MenuViewer viewer) {
        Player player = viewer.getPlayer();
        ChestShop shop = this.getLink(player);
    ...
    private void handleAdminShop(@NonNull MenuViewer viewer, boolean toAdmin) {
    ...
    private void handleRentCancel(@NonNull MenuViewer viewer) {
        Player player = viewer.getPlayer();
        ChestShop shop = this.getLink(player);
    ...
    private void handleBuyingMode(@NonNull MenuViewer viewer, @NonNull TradeType type) {
        ChestShop shop = this.getLink(viewer);
    ...
    public void loadConfiguration(@NonNull FileConfig config, @NonNull MenuLoader loader) {
        ...
        }, ItemOptions.builder().setVisibilityPolicy(viewer -> module.getSettings().isBankEnabled()).build()));
    }
}
