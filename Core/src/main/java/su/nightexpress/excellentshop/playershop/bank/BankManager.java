package su.nightexpress.excellentshop.playershop.bank;
...
import su.nightexpress.excellentshop.playershop.bank.data.BankDataManager;
...
public class BankManager extends AbstractManager<ShopPlugin> {
    ...
    public void loadBanks() {
        ...
        Players.getOnline().stream().map(Entity::getUniqueId).filter(Predicate.not(this::hasBank)).forEach(
            this::createBankAndLoad);
    ...
    public void saveBanks() {
        ...
        Set<Bank> banks = this.getBanks().stream().filter(Bank::isDirty).peek(Bank::markClean).collect(Collectors
            .toSet());
    ...
    public boolean withdrawFromBank(@NonNull Player player, @NonNull UUID target, @NonNull Currency currency,
                                    double amount) {
        Bank bank = this.getBankById(target);
        if (bank == null) {
            this.module.sendPrefixed(ChestLang.BANK_ERROR_NOT_EXIST, player);
