...
public class DisplayManager {
    ...
    public void render(@NonNull ChestShop shop) {
        ...
        int pindex = display.getTickCount() % itemInterval == 0 ? display.nextProductIndex() : display
            .getProductIndex();
        ...
        players.forEach(player -> {
            ...
            if (showcase != null && product != null) {
                display.fakeEntity(FakeType.SHOWCASE).ifPresent(entity -> {
                    this.adapter.sendShowcasePackets(player, entity, needSpawn, showcase.getDisplayItem()
                        .getItemStack());
                });
            }
