package su.nightexpress.excellentshop.playershop.dialog.impl;
...
public class BankManagementDialog extends Dialog<Bank> {
    ...
    private static final TextLocale          TITLE = ChestLang.builder("Dialog.Bank.Management.Title").text(title(
        "Bank", "Management"));
    ...
    private static final TextLocale ENTRY_CURRENCY = ChestLang.builder("Dialog.Bank.Management.Entry.Currency")
        .text(TagWrappers.SOFT_YELLOW.wrap("• ") + TagWrappers.WHITE.wrap(ShopPlaceholders.CURRENCY_NAME +
            ": ") + TagWrappers.SOFT_YELLOW.wrap(ShopPlaceholders.GENERIC_VALUE));
    ...
    private static final ButtonLocale BUTTON_DEPOSIT_SPECIFIED = ChestLang.builder(
        "Dialog.Bank.Management.Button.DepositSpecified")
    ...
    private static final ButtonLocale BUTTON_WITHDRAW_SPECIFIED = ChestLang.builder(
        "Dialog.Bank.Management.Button.WithdrawSpecified")
    ...
    private static final ButtonLocale BUTTON_WITHDRAW_ALL = ChestLang.builder(
        "Dialog.Bank.Management.Button.WithdrawAll")
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull Bank bank) {
        List<Currency> currencies = this.module.getAvailableCurrencies(player).stream().sorted(Comparator.comparing(
            Currency::getInternalId)).toList();
        List<WrappedSingleOptionEntry> entries = new ArrayList<>();

        currencies.forEach(currency -> {
            entries.add(new WrappedSingleOptionEntry(currency.getInternalId(), currency.getName(), this.module
        ...
        return Dialogs.create(builder -> builder
            ...
            .type(
                DialogTypes.multiAction(
                    DialogButtons.action(BUTTON_DEPOSIT_SPECIFIED).action(DialogActions.customClick(
                        ACTION_DEPOSIT_SPECIFIED)).build(),
                    DialogButtons.action(BUTTON_WITHDRAW_SPECIFIED).action(DialogActions.customClick(
                        ACTION_WITHDRAW_SPECIFIED)).build(),
                    DialogButtons.action(BUTTON_DEPOSIT_ALL).action(DialogActions.customClick(ACTION_DEPOSIT_ALL))
                        .build(),
                    DialogButtons.action(BUTTON_WITHDRAW_ALL).action(DialogActions.customClick(ACTION_WITHDRAW_ALL))
                        .build()
                )
