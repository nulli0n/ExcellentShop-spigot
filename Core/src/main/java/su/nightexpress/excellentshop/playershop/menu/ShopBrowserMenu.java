...
public class ShopBrowserMenu extends LinkedMenu<ShopPlugin, ShopBrowserMenu.Data> implements Filled<ChestShop>, ConfigBased {
    ...
    private enum SortType {
        ...
        SortType(@NonNull Comparator<ChestShop> comparator) {
            this.comparator = comparator;
        }
    }

    public ShopBrowserMenu(@NonNull ShopPlugin plugin, @NonNull ChestShopModule module) {
    ...
    public void openByPlayer(@NonNull Player player, @NonNull String ownerName) {
        this.open(player, ownerName, null, null);
    }

    public void openByItem(@NonNull Player player, @NonNull String itemSearch) {
        this.open(player, null, itemSearch, null);
    }

    public void openFromShop(@NonNull Player player, @NonNull ChestShop shop) {
    ...
    @Override
    @NonNull
    public MenuFiller<ChestShop> createFiller(@NonNull MenuViewer viewer) {
        Player player = viewer.getPlayer();
        Data data = this.getLink(player);
            ...
            .setItemCreator(shop -> {
                ...
                for (ChestProduct product : products) {
                    ...
                    for (String line : this.productEntry) {
                        productInfo.add(PlaceholderContext.builder().with(product.placeholders()).build().apply(line));
                    }
                    productCount++;
                }
                ...
                if (this.shopUseProductIcon) {
                    ...
                    item = NightItem.fromType(shop.isAccessible() ? shop.getBlock().getType() : Material.CHEST);
                }
                ...
                return item
                    ...
                    .replace(replacer -> replacer
                        .with(GENERIC_PRODUCTS, () -> String.join(TagWrappers.BR, productInfo))
