...
public class ShowcaseMenu extends LinkedMenu<ShopPlugin, ChestShop> implements Filled<Showcase>, ConfigBased {
    ...
    public ShowcaseMenu(@NonNull ShopPlugin plugin, @NonNull ChestShopModule module) {
    ...
    @Override
    @NonNull
    public MenuFiller<Showcase> createFiller(@NonNull MenuViewer viewer) {
        ...
        return MenuFiller.builder(this)
            ...
            .setItemCreator(showcase -> {
                NightItem item = shop.isShowcaseEnabled() ? showcase.getDisplayItem().setLore(this.showcaseLore)
                    .setDisplayName(showcase.getName()) : this.showcaseDisabled.copy();

                return item
                    .hideAllComponents()
                    .replacement(replacer -> replacer
                        .replace(ShopPlaceholders.GENERIC_TYPE, LangUtil.getSerializedName(showcase.getDisplayItem()
                            .getMaterial()))
                    );
            })
    ...
    protected void onItemPrepare(@NonNull MenuViewer viewer, @NonNull MenuItem menuItem, @NonNull NightItem item) {
        ...
        item.replace(replacer -> replacer.with(this.getLink(viewer).placeholders()));
    }

    private void handleHologram(@NonNull MenuViewer viewer) {
        this.handleChange(viewer, shop -> shop.setHologramEnabled(!shop.isHologramEnabled()));
    }

    private void handleShowcaseToggle(@NonNull MenuViewer viewer) {
        this.handleChange(viewer, shop -> shop.setShowcaseEnabled(!shop.isShowcaseEnabled()));
    }

    private void handleShowcaseReset(@NonNull MenuViewer viewer) {
        this.handleChange(viewer, shop -> shop.setShowcaseId(null));
    }

    private void handleChange(@NonNull MenuViewer viewer, @NonNull Consumer<ChestShop> consumer) {
    ...
    public void loadConfiguration(@NonNull FileConfig config, @NonNull MenuLoader loader) {
        ...
        this.showcaseSlots = ConfigValue.create("Showcase.Slots",
        ...
        loader.addDefaultItem(NightItem.fromType(Material.BARRIER)
            ...
            .setSlots(53)
            .setPriority(10)
            .setHandler(new ItemHandler("showcase_reset", (viewer, event) -> this.handleShowcaseReset(
