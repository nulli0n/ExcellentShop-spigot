package su.nightexpress.excellentshop.playershop.menu;
...
public class ProductsMenu extends LinkedMenu<ShopPlugin, ProductsMenu.Data> implements Filled<ChestProduct>, ConfigBased {
    ...
    public ProductsMenu(@NonNull ShopPlugin plugin, @NonNull ChestShopModule module) {
        super(plugin, MenuType.GENERIC_9X6, BLACK.wrap("Shop Products"));
        this.module = module;
    }

    public record Data(@NonNull ChestShop shop, ChestProduct product, int index) {

        public boolean test(@NonNull Predicate<ChestProduct> predicate) {
    ...
    public void open(@NonNull Player player, @NonNull ChestShop shop) {
    ...
    private void open(@NonNull Player player, @NonNull ChestShop shop, @Nullable ChestProduct product, int index) {
        this.open(player, new Data(shop, product, index));
    }

    @Override
    @NonNull
    public MenuFiller<ChestProduct> createFiller(@NonNull MenuViewer viewer) {
    ...
    protected void onItemPrepare(@NonNull MenuViewer viewer, @NonNull MenuItem menuItem, @NonNull NightItem item) {
        ...
        item.replace(replacer -> {
            replacer.with(shop.placeholders());
            if (product != null) replacer.with(product.placeholders());
        });
    }
    ...
    private void handlePrice(@NonNull MenuViewer viewer, @NonNull TradeType tradeType) {
        Player player = viewer.getPlayer();
        Data data = this.getLink(player);
    ...
    private void handlePriceOff(@NonNull MenuViewer viewer, @NonNull TradeType tradeType) {
        Player player = viewer.getPlayer();
        Data data = this.getLink(player);
    ...
    private void handleCurrency(@NonNull MenuViewer viewer) {
        Player player = viewer.getPlayer();
        Data data = this.getLink(player);
    ...
    private void handleRestock(@NonNull MenuViewer viewer, boolean all) {
        Player player = viewer.getPlayer();
        Data data = this.getLink(player);
        ChestProduct product = data.product;
    ...
    private void handleWithdraw(@NonNull MenuViewer viewer, boolean all) {
        Player player = viewer.getPlayer();
        Data data = this.getLink(player);
        ChestProduct product = data.product;
        ...
        if (all) {
            ...
            this.module.withdrawFromShop(player, product, productStock);
            this.runNextTick(() -> this.flush(viewer));
            return;
        }
    ...
    private void handleRemove(@NonNull MenuViewer viewer) {
        Player player = viewer.getPlayer();
        Data data = this.getLink(player);
    ...
    public void loadConfiguration(@NonNull FileConfig config, @NonNull MenuLoader loader) {
        ...
        this.highlightIcon = ConfigValue.create("Product.Icon.Highlight", NightItem.fromType(
            Material.LIME_STAINED_GLASS_PANE)
            .setDisplayName(GREEN.wrap("← " + BOLD.wrap("Selected Product")))
            .hideAllComponents()
        ).read(config);

        loader.addDefaultItem(MenuItem.buildReturn(this, 49, (viewer, event) -> this.handleReturn(viewer)).setPriority(
            10).build());

        loader.addDefaultItem(NightItem.fromType(Material.BLACK_STAINED_GLASS_PANE).setHideTooltip(true).toMenuItem()
            .setSlots(1, 10, 19, 28, 37, 45, 46, 47, 48, 49, 50, 51, 52, 53));
        loader.addDefaultItem(NightItem.fromType(Material.GRAY_STAINED_GLASS_PANE).setHideTooltip(true).toMenuItem()
            .setSlots(0, 36));

        // Currency button.

        loader.addDefaultItem(NightItem.fromType(Material.EMERALD)
            .setDisplayName(LIGHT_GREEN.wrap(BOLD.wrap("Currency")) + " " + GRAY.wrap("[" + WHITE.wrap(
                PRODUCT_CURRENCY) + "]"))
            .setLore(Lists.newList(
            ...
            .setPriority(10)
            .setSlots(23)
            .setHandler(new ItemHandler("set_currency", (viewer, event) -> this.handleCurrency(viewer), ItemOptions
                .builder()
                .setVisibilityPolicy(viewer -> this.getLink(viewer).hasProduct() && this.module.getAvailableCurrencies(
                    viewer.getPlayer()).size() > 1)
                .build()
            )));
        ...
        loader.addDefaultItem(NightItem.fromType(Material.FIREWORK_STAR)
            ...
            .setPriority(10)
            .setSlots(12)
            .setHandler(new ItemHandler("add_buy_price", (viewer, event) -> this.handlePrice(viewer,
                TradeType.BUY), ItemOptions.builder()
                    .setVisibilityPolicy(viewer -> this.getLink(viewer).test(product -> !product.hasBuyPrice()))
                    .build()
            )));

        loader.addDefaultItem(NightItem.fromType(Material.FIREWORK_STAR)
            .setDisplayName(GREEN.wrap(BOLD.wrap("Buy Price")) + " " + GRAY.wrap("[" + WHITE.wrap(
                PRODUCT_PRICE_FORMATTED.apply(TradeType.BUY)) + "]"))
            .setLore(Lists.newList(
            ...
            .setPriority(10)
            .setSlots(12)
            .setHandler(new ItemHandler("change_buy_price", (viewer, event) -> this.handlePrice(viewer,
                TradeType.BUY), ItemOptions.builder()
        ...
        loader.addDefaultItem(NightItem.fromType(Material.REPEATER)
            ...
            .setPriority(10)
            .setSlots(13)
            .setHandler(new ItemHandler("disable_buying", (viewer, event) -> this.handlePriceOff(viewer,
                TradeType.BUY), ItemOptions.builder()
                    .setVisibilityPolicy(viewer -> viewer.getPlayer().hasPermission(ChestPerms.DISABLE_BUYING) && this
                        .getLink(viewer).test(ChestProduct::hasBuyPrice))
                    .build()
            )));
        ...
        loader.addDefaultItem(NightItem.fromType(Material.FIREWORK_STAR)
            ...
            .setPriority(10)
            .setSlots(15)
            .setHandler(new ItemHandler("add_sell_price", (viewer, event) -> this.handlePrice(viewer,
                TradeType.SELL), ItemOptions.builder()
                    .setVisibilityPolicy(viewer -> viewer.getPlayer().hasPermission(ChestPerms.DISABLE_SELLING) && this
                        .getLink(viewer).test(product -> !product.hasSellPrice()))
                    .build()
            )));

        loader.addDefaultItem(NightItem.fromType(Material.FIREWORK_STAR)
            .setDisplayName(RED.wrap(BOLD.wrap("Sell Price")) + " " + GRAY.wrap("[" + WHITE.wrap(PRODUCT_PRICE_FORMATTED
                .apply(TradeType.SELL)) + "]"))
            .setLore(Lists.newList(
            ...
            .setPriority(10)
            .setSlots(15)
            .setHandler(new ItemHandler("change_sell_price", (viewer, event) -> this.handlePrice(viewer,
                TradeType.SELL), ItemOptions.builder()
        ...
        loader.addDefaultItem(NightItem.fromType(Material.REPEATER)
            ...
            .setPriority(10)
            .setSlots(16)
            .setHandler(new ItemHandler("disable_selling", (viewer, event) -> this.handlePriceOff(viewer,
                TradeType.SELL), ItemOptions.builder()
                    .setVisibilityPolicy(viewer -> viewer.getPlayer().hasPermission(ChestPerms.DISABLE_SELLING) && this
                        .getLink(viewer).test(ChestProduct::hasSellPrice))
                    .build()
            )));

        // Stock buttons.

        loader.addDefaultItem(NightItem.fromType(Version.isAtLeast(
            Version.MC_1_21_4) ? Material.LIGHT_BLUE_BUNDLE : Material.BUNDLE)
            .setDisplayName(LIGHT_BLUE.wrap(BOLD.wrap("Restock")) + " " + GRAY.wrap("[" + WHITE.wrap("Custom Amount") +
                "]"))
            .setLore(Lists.newList(
                GRAY.wrap(LIGHT_BLUE.wrap("✔ ") + "Stock: " + LIGHT_BLUE.wrap(PRODUCT_STOCK) + "/" + LIGHT_BLUE.wrap(
                    PRODUCT_CAPACITY)),
                "",
            ...
            .setPriority(10)
            .setSlots(30)
            .setHandler(new ItemHandler("restock_custom", (viewer, event) -> this.handleRestock(viewer,
                false), ItemOptions.builder()
                    .setVisibilityPolicy(viewer -> this.getLink(viewer).hasProduct() && !this.getLink(viewer).shop
                        .isAdminShop())
                    .build()
            )));
        ...
        loader.addDefaultItem(NightItem.fromType(Material.WATER_BUCKET)
            ...
            .setPriority(10)
            .setSlots(31)
            .setHandler(new ItemHandler("restock_all", (viewer, event) -> this.handleRestock(viewer, true), ItemOptions
                .builder()
                .setVisibilityPolicy(viewer -> this.getLink(viewer).hasProduct() && !this.getLink(viewer).shop
                    .isAdminShop())
                .build()
        ...
        loader.addDefaultItem(NightItem.fromType(Version.isAtLeast(
            Version.MC_1_21_4) ? Material.ORANGE_BUNDLE : Material.BUNDLE)
            .setDisplayName(LIGHT_ORANGE.wrap(BOLD.wrap("Withdraw")) + " " + GRAY.wrap("[" + WHITE.wrap(
                "Custom Amount") + "]"))
            .setLore(Lists.newList(
                GRAY.wrap(LIGHT_ORANGE.wrap("✔ ") + "Stock: " + LIGHT_ORANGE.wrap(PRODUCT_STOCK) + "/" + LIGHT_ORANGE
                    .wrap(PRODUCT_SPACE)),
                "",
            ...
            .setPriority(10)
            .setSlots(33)
            .setHandler(new ItemHandler("withdraw_custom", (viewer, event) -> this.handleWithdraw(viewer,
                false), ItemOptions.builder()
                    .setVisibilityPolicy(viewer -> this.getLink(viewer).hasProduct() && !this.getLink(viewer).shop
                        .isAdminShop())
                    .build()
        ...
        loader.addDefaultItem(NightItem.fromType(Material.LAVA_BUCKET)
            ...
            .setPriority(10)
            .setSlots(34)
            .setHandler(new ItemHandler("withdraw_all", (viewer, event) -> this.handleWithdraw(viewer,
        ...
        loader.addDefaultItem(NightItem.fromType(Material.BARRIER)
            ...
            .setPriority(10)
            .setSlots(53)
            .setHandler(new ItemHandler("remove_item", (viewer, event) -> this.handleRemove(viewer), ItemOptions
                .builder()
