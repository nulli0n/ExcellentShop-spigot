...
public class PlayerShopDialogs extends SimpleManager<ShopPlugin> {
    ...
    public PlayerShopDialogs(@NonNull ShopPlugin plugin, @NonNull ChestShopModule module) {
        super(plugin);
        this.module = module;
    }
    ...
    public void openShopNameDialog(@NonNull Player player, @NonNull ChestShop shop) {
        Dialogs.showDialog(player, this.createShopNameDialog(shop));
    }

    @NonNull
    private WrappedDialog createShopNameDialog(@NonNull ChestShop shop) {
        return Dialogs.create(builder -> builder
            .base(DialogBases.builder("Shop Name")
                .body(DialogBodies.plainMessage("Enter the name for your shop.<br>Max length is " +
                    ChestConfig.SHOP_MAX_NAME_LENGTH.get()))
