...
import su.nightexpress.excellentshop.shop.ShopModuleSettings;
...
public class VirtualSettings extends AbstractConfig implements ShopModuleSettings {
    ...
    private final ConfigProperty<Map<TradeStatus, List<String>>> guiProductDisplayMasterInfo = this.addProperty(
        TRADE_STATUS_LORE_TYPE, "GUI.Product.Display.Master",
        getDefaultProducePriceInfo(),
        ""
    );

    private final ConfigProperty<String> guiProductDisplayComponentPriceTrendUp = this.addProperty(ConfigTypes.STRING,
        "GUI.Product.Display.Component.price_trend.up",
        TagWrappers.GREEN.wrap("↑ " + ShopPlaceholders.GENERIC_TREND + "%")
    );

    private final ConfigProperty<String> guiProductDisplayComponentPriceTrendDown = this.addProperty(ConfigTypes.STRING,
        "GUI.Product.Display.Component.price_trend.down",
    ...
    private final ConfigProperty<Boolean> guiBuyingCloseAfterPurcahse = this.addProperty(ConfigTypes.BOOLEAN,
        "GUI.Buying.Close-After-Purchase",
        false,
    ...
    private static Map<TradeStatus, List<String>> getDefaultProducePriceInfo() {
        ...
        List<String> buyInfo = Lists.newList(
            ...
            "<if_dynamic_price>" + dgray.wrap(" » ") + gray.wrap("Trend:") + " " + white.wrap("{buy_price_trend}") +
                "</if_dynamic_price>"
        );
        ...
        List<String> sellInfo = Lists.newList(
            ...
            "<if_dynamic_price>" + dgray.wrap(" » ") + gray.wrap("Trend:") + " " + white.wrap("{sell_price_trend}") +
                "</if_dynamic_price>"
        );

        List<String> stockInfo = Lists.newList(
            "<if_has_stock>" + TagWrappers.SOFT_AQUA.wrap("┃ Stock:") + "</if_has_stock>",
            "<if_has_stock>" + dgray.wrap(" » ") + gray.wrap("Current:") + " " + white.wrap(
                ShopPlaceholders.PRODUCT_STOCK) + gray.wrap("/") + white.wrap(ShopPlaceholders.PRODUCT_CAPACITY) +
                "</if_has_stock>",
            "<if_stock_resettable>" + dgray.wrap(" » ") + gray.wrap("Auto-Refill:") + " " + white.wrap(
                "{stock_reset_time}") + "</if_stock_resettable>"
        );

        List<String> limitsInfo = Lists.newList(
            "<if_has_limits>" + TagWrappers.ORANGE.wrap("┃ Limits:") + "</if_has_limits>",
            "<if_has_buy_limit>" + dgray.wrap(" » ") + gray.wrap("Buy Limit:") + " " + orange.wrap(
                "{buy_limit_current}" + gray.wrap("/") + ShopPlaceholders.PRODUCT_BUY_LIMIT) + "</if_has_buy_limit>",
            "<if_has_sell_limit>" + dgray.wrap(" » ") + gray.wrap("Sell Limit:") + " " + orange.wrap(
                "{sell_limit_current}" + gray.wrap("/") + ShopPlaceholders.PRODUCT_SELL_LIMIT) + "</if_has_sell_limit>",
            "<if_limit_resettable>" + dgray.wrap(" » ") + gray.wrap("Auto-Reset:") + " " + white.wrap(
                "{limit_reset_time}") + "</if_limit_resettable>"
        );
        ...
        buyableOnly.add(dgray.wrap(" » ") + blue.wrap("Left-Click") + " " + gray.wrap("→") + " " + white.wrap(
            "Open Buy Menu"));
        buyableOnly.add(dgray.wrap(" » ") + blue.wrap("Right-Click") + " " + gray.wrap("→") + " " + white.wrap(
            "Buy x 1"));
        buyableOnly.add(dgray.wrap(" » ") + blue.wrap("Key " + TagWrappers.KEY.apply("key.hotbar.1") + "-" +
            TagWrappers.KEY.apply("key.hotbar.9")) + " " + gray.wrap("→") + " " + white.wrap("Quick Buy"));
        ...
        sellableOnly.add(dgray.wrap(" » ") + blue.wrap("Left-Click") + " " + gray.wrap("→") + " " + white.wrap(
            "Open Sell Menu"));
        sellableOnly.add(dgray.wrap(" » ") + blue.wrap("Right-Click") + " " + gray.wrap("→") + " " + white.wrap(
            "Sell x 1"));
        sellableOnly.add(dgray.wrap(" » ") + blue.wrap("Key " + TagWrappers.KEY.apply("key.hotbar.1") + "-" +
            TagWrappers.KEY.apply("key.hotbar.9")) + " " + gray.wrap("→") + " " + white.wrap("Quick Sell"));
        sellableOnly.add(dgray.wrap(" » ") + blue.wrap("Control + " + TagWrappers.KEY.apply("key.drop")) + " " + gray
            .wrap("→") + " " + white.wrap("Sell All"));
        ...
        both.add(dgray.wrap(" » ") + blue.wrap("Left-Click") + " " + gray.wrap("→") + " " + white.wrap(
            "Open Buy Menu"));
        both.add(dgray.wrap(" » ") + blue.wrap("Right-Click") + " " + gray.wrap("→") + " " + white.wrap(
            "Open Sell Menu"));
        both.add(dgray.wrap(" » ") + blue.wrap(TagWrappers.KEY.apply("key.sneak") + " + LMB") + " " + gray.wrap("→") +
            " " + white.wrap("Buy x 1"));
        both.add(dgray.wrap(" » ") + blue.wrap(TagWrappers.KEY.apply("key.sneak") + " + RMB") + " " + gray.wrap("→") +
            " " + white.wrap("Sell x 1"));
        both.add(dgray.wrap(" » ") + blue.wrap("Control + " + TagWrappers.KEY.apply("key.drop")) + " " + gray.wrap(
            "→") + " " + white.wrap("Sell All"));
