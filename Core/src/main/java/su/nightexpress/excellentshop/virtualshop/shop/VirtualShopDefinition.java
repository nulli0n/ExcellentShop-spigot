...
public class VirtualShopDefinition implements Writeable {
    ...
    @NonNull
    public VirtualShopDefinition load(@NonNull FileConfig config, @NonNull String path) {
        ...
        if (config.contains(path + ".Layout.Name")) {
            String oldDefault = config.getString(path + ".Layout.Name", ShopPlaceholders.DEFAULT);
        ...
        this.setMenuSlots(IntStream.of(config.getIntArray(path + ".MainMenu.Slot")).boxed().collect(Collectors
            .toSet()));
        ...
        config.getSection(path + ".Layout.ByPage").forEach(sId -> {
            ...
            this.pageLayouts.put(page, config.getString(path + ".Layout.ByPage." + sId, ShopPlaceholders.DEFAULT));
        });
    ...
    public String getLayout(int page) {
        return this.paginatedLayouts && this.pageLayouts.containsKey(page) ? this.pageLayouts.get(
            page) : this.pageLayouts.computeIfAbsent(DEFAULT_LAYOUT_PAGE, k -> ShopPlaceholders.DEFAULT);
    }
    ...
    public void removePageLayout(int page) {
        if (page == DEFAULT_LAYOUT_PAGE) {
            this.setPageLayout(page, ShopPlaceholders.DEFAULT);
        }
        else {
