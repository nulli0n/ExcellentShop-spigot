...
public class Rotation implements Writeable {
    ...
    private static final NightItem DEFAULT_ICON = NightItem.asCustomHead(
        "a70216baf1b9675f805dfdf95db043afe6f881c82b25937e46b15068e8f3e882");
    ...
    private boolean      notifyOnRotation;

    public Rotation(@NonNull UUID globalId, @NonNull String id, @NonNull VirtualShop shop) {
    ...
    public void load(@NonNull FileConfig config, @NonNull String path) {
        config.getSection(path + ".SlotsByPage").forEach(pageStr -> {
            ...
            Set<Integer> slots = IntStream.of(config.getIntArray(path + ".SlotsByPage." + pageStr)).boxed().collect(
        ...
        this.setNotifyOnRotation(config.get(ConfigTypes.BOOLEAN, path + ".NotifyOnRotation", true));
        ...
        for (String sDay : config.getSection(path + ".Refresh.Fixed")) {
            ...
            TreeSet<LocalTime> times = new TreeSet<>(ShopUtils.parseTimes(config.getStringList(path +
                ".Refresh.Fixed." + sDay)));
    ...
    public void write(@NonNull FileConfig config, @NonNull String path) {
        ...
        config.set(path + ".NotifyOnRotation", this.notifyOnRotation); // TODO Editor impl
        ...
        this.rotationTimes.forEach((day, times) -> {
            config.set(path + ".Refresh.Fixed." + day.name(), times.stream().map(time -> time.format(
                ShopUtils.TIME_FORMATTER)).toList());
        });
    }

    @NonNull
    public UnaryOperator<String> replacePlaceholders() {
        return ShopPlaceholders.ROTATION.replacer(this);
    }

    @NonNull
    public List<RotationItemData> generateRotationProducts() {
        List<RotationItemData> itemData = new ArrayList<>();
        ...
        for (int page = 1; page < this.shop.getPages() + 1; page++) {
            ...
            while (amount > 0 && !itemWeights.isEmpty()) {
                ...
                itemData.add(new RotationItemData(page, item.getProductId()));
    ...
    public long createNextRotationTimestamp() {
        if (this.rotationType == RotationType.INTERVAL) {
    ...
    @Nullable
    private LocalTime findTime(@NonNull LocalDate dateLookup) {
    ...
    public void setSlots(@NonNull Set<Integer> slots, int page) {
    ...
    public void addItem(@NonNull RotationItem item) {
        this.itemMap.put(item.getProductId(), item);
    }

    public void removeItem(@NonNull RotationItem item) {
        this.itemMap.remove(item.getProductId());
    }

    public boolean hasProduct(@NonNull VirtualProduct product) {
        return this.hasItem(product.getId());
    }

    public boolean hasItem(@NonNull String productId) {
    ...
    @Nullable
    public RotationItem getItemById(@NonNull String id) {
    ...
    public void setRotationType(@NonNull RotationType rotationType) {
    ...
    public boolean isNotifyOnRotation() {
        return this.notifyOnRotation;
    }

    public void setNotifyOnRotation(boolean notifyOnRotation) {
        this.notifyOnRotation = notifyOnRotation;
    }
    ...
    @NonNull
    public TreeSet<LocalTime> getRotationTimes(@NonNull DayOfWeek day) {
