...
public class RotationListMenu extends LinkedMenu<ShopPlugin, VirtualShop> implements Filled<Rotation> {
    ...
    public RotationListMenu(@NonNull ShopPlugin plugin, @NonNull VirtualShopModule module) {
        ...
        this.addItem(Material.ANVIL, VirtualLocales.ROTATION_CREATE, 41, (viewer, event, shop) -> {
            this.handleInput(Dialog.builder(viewer, VirtualLang.EDITOR_ENTER_ROTATION_ID.text(), input -> {
                ...
                Rotation rotation = new Rotation(UUID.randomUUID(), id, shop);
    ...
    @Override
    @NonNull
    public MenuFiller<Rotation> createFiller(@NonNull MenuViewer viewer) {
