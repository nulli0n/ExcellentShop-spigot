...
public class ShopDescriptionDialog extends Dialog<VirtualShop> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.ShopDescription.Title").text(title(
        "Shop", "Description"));
    private static final DialogElementLocale BODY  = VirtualLang.builder("Dialog.ShopDescription.Body").dialogElement(
        ...
        "",
        "Feel free to use the " + SOFT_YELLOW.wrap(VIRTUAL_SHOP_DESCRIPTION) +
            " placeholder in configuration files to \"insert\" shop description in GUIs and messages.",
        ""
    );

    private static final DialogElementLocale INPUT_DESCRIPTION = VirtualLang.builder(
        "Dialog.ShopDescription.Input.Text").dialogElement("Description");
