package su.nightexpress.excellentshop.virtualshop.shop.editor;
...
import su.nightexpress.excellentshop.virtualshop.core.*;
...
public class ShopOptionsMenu extends LinkedMenu<ShopPlugin, VirtualShop> {
    ...
    public ShopOptionsMenu(@NonNull ShopPlugin plugin, @NonNull VirtualShopModule module) {
        super(plugin, MenuType.GENERIC_9X6, VirtualLang.EDITOR_TITLE_SHOP_SETTINGS.text());

        this.addItem(NightItem.fromType(Material.GRAY_STAINED_GLASS_PANE).setHideTooltip(true).toMenuItem().setPriority(
            -1)
            .setSlots(0, 1, 2, 3, 4, 5, 6, 7, 8, 17, 26, 35, 9, 18, 27, 36, 37, 38, 39, 40, 41, 42, 43, 44));
        this.addItem(NightItem.fromType(Material.BLACK_STAINED_GLASS_PANE).setHideTooltip(true).toMenuItem()
            .setPriority(-1)
        ...
        this.addItem(NightItem.fromType(Material.COMMAND_BLOCK), VirtualIconsLang.ICON_SHOP_ALIASES, 13, (viewer, event,
                                                                                                          shop) -> {
        ...
        this.addItem(NightItem.fromType(Material.GLOW_ITEM_FRAME), VirtualLocales.SHOP_EDIT_LAYOUTS, 28, (viewer, event,
                                                                                                          shop) -> {
            module.openShopPageLayoutsDialog(viewer.getPlayer(), shop, () -> this.flush(viewer));
        });

        this.addItem(NightItem.fromType(Material.LIME_DYE), VirtualLocales.SHOP_EDIT_BUYING, 29, (viewer, event,
                                                                                                  shop) -> {
            shop.setBuyingAllowed(!shop.isBuyingAllowed());
            this.saveAndFlush(viewer, shop);
        }, ItemOptions.builder().setDisplayModifier((viewer, item) -> item.setMaterial(this.getLink(viewer)
            .isBuyingAllowed() ? Material.LIME_DYE : Material.GRAY_DYE)).build());

        this.addItem(NightItem.fromType(Material.LIME_DYE), VirtualLocales.SHOP_EDIT_SELLING, 30, (viewer, event,
                                                                                                   shop) -> {
            shop.setSellingAllowed(!shop.isSellingAllowed());
            this.saveAndFlush(viewer, shop);
        }, ItemOptions.builder().setDisplayModifier((viewer, item) -> item.setMaterial(this.getLink(viewer)
            .isSellingAllowed() ? Material.LIME_DYE : Material.GRAY_DYE)).build());


        this.addItem(NightItem.fromType(Material.CHEST), VirtualLocales.SHOP_EDIT_PRODUCTS_NORMAL, 32, (viewer, event,
                                                                                                        shop) -> {
            this.runNextTick(() -> module.openNormalProducts(viewer.getPlayer(), shop));
        });

        this.addItem(NightItem.fromType(Material.ENDER_CHEST), VirtualLocales.SHOP_EDIT_PRODUCTS_ROTATING, 33, (viewer,
                                                                                                                event,
                                                                                                                shop) -> {
            this.runNextTick(() -> module.openRotatingsProducts(viewer.getPlayer(), shop));
        });

        this.addItem(NightItem.fromType(Material.CLOCK), VirtualLocales.SHOP_EDIT_ROTATIONS, 34, (viewer, event,
                                                                                                  shop) -> {
    ...
    private void saveAndFlush(@NonNull MenuViewer viewer, @NonNull VirtualShop shop) {
        shop.markDirty();
    ...
    protected void onItemPrepare(@NonNull MenuViewer viewer, @NonNull MenuItem menuItem, @NonNull NightItem item) {
        ...
        item.replace(builder -> builder
            ...
            .with(ShopPlaceholders.VIRTUAL_SHOP_PERMISSION_REQUIRED, () -> CoreLang.STATE_YES_NO.get(shop
                .isPermissionRequired()))
            .with(ShopPlaceholders.VIRTUAL_SHOP_MENU_SLOTS, () -> !shop.hasMenuSlots() ? CoreLang.STATE_ENABLED_DISALBED
                .get(false) : shop.getMenuSlots().stream().map(i -> "#" + i).collect(Collectors.joining(", ")))
            .with(ShopPlaceholders.VIRTUAL_SHOP_ALIASES, () -> !shop.hasAliases() ? Lang.OTHER_UNDEFINED.text() : shop
                .getSlashedAliases().stream().map(CoreLang::goodEntry).collect(Collectors.joining(TagWrappers.BR)))
        );
    }
