package su.nightexpress.excellentshop.virtualshop.product.editor;
...
public class ProductOptionsMenu extends AbstractObjectMenu<VirtualProduct> {
    ...
    private static final IconLocale LOCALE_ITEM = VirtualLang.iconBuilder("UI.Editor.Product.TargetItem")
        .name("Item")
        .appendInfo("This is what the player", "receives when " + TagWrappers.GREEN.wrap("buying") + " and gives",
            "away when " + TagWrappers.RED.wrap("selling") + ".").br()
    ...
    private static final IconLocale LOCALE_ICON = VirtualLang.iconBuilder("UI.Editor.Product.Icon")
        .name("Icon")
        .appendInfo("Use a custom icon with", "a name and lore to provide", "players with a better overview",
            "of this product.").br()
        .appendClick("Drag'n'Drop to replace")
        .build();
    ...
    private static final IconLocale LOCALE_PERMISSIONS = VirtualLang.iconBuilder(
        "UI.Editor.Product.PermissionRequirements")
        ...
        this.addDefaultButton("product_type", MenuItem.button()
            .defaultState(ItemState.builder()
                ...
                .displayModifier((context, item) -> item.replace(builder -> builder
                    .with(ShopPlaceholders.GENERIC_TYPE, () -> Lang.CONTENT_TYPE.getLocalized(this.getObject(context)
                        .getContent().type()))
                ))
            ...
            .defaultState(ItemState.builder()
                ...
                .displayModifier((context, item) -> item.replace(builder -> builder
                    .with(ShopPlaceholders.GENERIC_BUY, () -> {
                        ...
                        return product.isBuyable() ? product.getCurrency().format(product
                            .getBuyPrice()) : Lang.OTHER_N_A.text();
                    })
                    .with(ShopPlaceholders.GENERIC_SELL, () -> {
                        VirtualProduct product = this.getObject(context);
                        return product.isSellable() ? product.getCurrency().format(product
                            .getSellPrice()) : Lang.OTHER_N_A.text();
                    })
                    .with(ShopPlaceholders.GENERIC_TYPE, () -> Lang.PRICE_TYPES.getLocalized(this.getObject(context)
                        .getPricingType()))
                ))
    ...
    private void addItemContentButtons(@NonNull VirtualProduct product, @NonNull ItemContent content,
                                       @NonNull List<MenuItem> items) {
            ...
            .defaultState(ItemState.builder()
                ...
                .displayModifier((ctx, item) -> {
                    ...
                    item.replace(builder -> builder
                        .with(ShopPlaceholders.GENERIC_STATE, () -> CoreLang.STATE_ENABLED_DISALBED.get(content
                            .isCompareNbt()))
                    );
                })
    ...
    private void addCommandContentButtons(@NonNull VirtualProduct product, @NonNull CommandContent content,
                                          @NonNull List<MenuItem> items) {
        items.add(MenuItem.button()
            .defaultState(ItemState.builder()
        ...
        items.add(MenuItem.button()
            .defaultState(ItemState.builder()
                .icon(NightItem.fromType(Material.COMMAND_BLOCK_MINECART).hideAllComponents().localized(
                    LOCALE_COMMANDS))
    ...
    private void handleContentType(@NonNull ActionContext context) {
        this.module.openProductTypeDialog(context.getPlayer(), this.getObject(context), () -> context.getViewer()
            .refresh());
    }
