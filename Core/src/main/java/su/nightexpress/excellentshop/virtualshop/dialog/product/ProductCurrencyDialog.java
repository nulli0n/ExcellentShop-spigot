...
public class ProductCurrencyDialog extends Dialog<VirtualProduct> {
    ...
    private static final TextLocale TITLE = VirtualLang.builder("Dialog.ProductCurrency.Title").text(title("Product",
        "Currency"));
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull VirtualProduct product) {
        ...
        this.module.getEnabledCurrencies().stream().sorted(Comparator.comparing(Currency::getInternalId)).forEach(
            currency -> {
                buttons.add(DialogButtons.action(currency.getName())
                    .action(DialogActions.customClick(ACTION_CURRENCY, NightNbtHolder.builder().put(JSON_CURRENCY_ID,
                        currency.getInternalId()).build()))
                    .build());
            });
        ...
        return Dialogs.create(builder -> builder
            ...
            .handleResponse(ACTION_CURRENCY, (viewer, identifier, nbtHolder) -> {
                ...
                Currency currency = nbtHolder.getText(JSON_CURRENCY_ID).map(EconomyBridge.api()::getCurrency).orElse(
                    null);
