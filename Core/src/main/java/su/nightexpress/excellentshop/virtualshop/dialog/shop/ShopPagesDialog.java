...
import su.nightexpress.excellentshop.virtualshop.shop.VirtualShopDefinition;
...
public class ShopPagesDialog extends Dialog<VirtualShop> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.ShopPages.Title").text(title("Shop",
        "Number of Pages"));
    private static final DialogElementLocale BODY  = VirtualLang.builder("Dialog.ShopPages.Body").dialogElement(
        ...
        "",
        SOFT_RED.wrap("→") + "Ensure that " + SOFT_RED.wrap("Shop Layout(s)") + " config file contains " + SOFT_RED
            .wrap("page buttons") + "!",
        ""
    );

    private static final DialogElementLocale INPUT_PAGES = VirtualLang.builder("Dialog.ShopPages.Input.Pages")
        .dialogElement("Pages");
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull VirtualShop shop) {
        return Dialogs.create(builder -> builder
            .base(DialogBases.builder(TITLE)
                ...
                .inputs(
                    DialogInputs.numberRange(JSON_PAGES, INPUT_PAGES, VirtualShopDefinition.MIN_PAGES,
                        VirtualShopDefinition.MAX_PAGES)
