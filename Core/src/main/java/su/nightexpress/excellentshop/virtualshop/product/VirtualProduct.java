...
public class VirtualProduct extends AbstractProduct<VirtualShop> implements Writeable {
    ...
    public VirtualProduct(@NonNull DataManager dataManager, @NonNull UUID globalId, @NonNull String id,
                          @NonNull VirtualShop shop) {
    ...
    public void load(@NonNull FileConfig config, @NonNull String path) throws ProductLoadException {
        ...
        this.setPricing(PriceFactory.read(config, path + ".Price"));
    ...
    protected double applyPriceModifiers(@NonNull TradeType tradeType, double currentPrice, @Nullable Player player) {
        if (tradeType == TradeType.SELL && player != null) {
    ...
    public PriceData getPriceData() {
        if (this.getPricingType() == PriceType.FLAT || this.getPricingType() == PriceType.PLAYER_AMOUNT)
