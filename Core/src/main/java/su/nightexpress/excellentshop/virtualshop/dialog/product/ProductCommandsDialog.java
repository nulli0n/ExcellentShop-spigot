...
public class ProductCommandsDialog extends Dialog<VirtualProduct> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.ProductCommands.Title").text(title(
        "Produc", "Commands"));
    private static final DialogElementLocale BODY  = VirtualLang.builder("Dialog.ProductCommands.Body").dialogElement(
        ...
        SOFT_YELLOW.wrap("→") + " The " + SOFT_YELLOW.wrap(GENERIC_AMOUNT) + " placeholder for the " + SOFT_YELLOW.wrap(
            "selected quantity") + ".",
        SOFT_YELLOW.wrap("→") + " The " + SOFT_YELLOW.wrap(PLAYER_NAME) + " placeholder for the " + SOFT_YELLOW.wrap(
            "player name") + ".",
        "",
        GREEN.wrap("✔") + " Internal " + GREEN.wrap("Shop") + " and " + GREEN.wrap("Product") +
            " placeholders are available.",
        GREEN.wrap("✔") + " External " + GREEN.wrap(PAPI.NAME) + " placeholders are available."
    );

    private static final DialogElementLocale INPUT_COMMANDS = VirtualLang.builder("Dialog.ProductCommands.Input.Text")
        .dialogElement(300, "Commands");
    ...
    private void setShopAliases(@NonNull VirtualProduct product, @NonNull CommandContent content,
                                @NonNull String[] aliases) {
        content.setCommands(Arrays.stream(aliases).map(s -> s.startsWith("/") && s.length() >= 2 ? s.substring(1) : s)
            .toList());
