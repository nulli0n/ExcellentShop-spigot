...
public class ShopMenuSlotsDialog extends Dialog<VirtualShop> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.ShopSlots.Title").text(title("Shop",
        "Menu Slots"));
    private static final DialogElementLocale BODY  = VirtualLang.builder("Dialog.ShopSlots.Body").dialogElement(
        ...
        "",
        "You can assign " + SOFT_YELLOW.wrap("multiple slots") + " for the same shop " + GRAY.wrap(
            "(useful for GUIs with custom textures)"),
        "",
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull VirtualShop shop) {
        ...
        this.module.getShops().stream().filter(other -> other != shop).forEach(other -> occupiedSlots.addAll(other
            .getMenuSlots()));
        ...
        allowedSlots.forEach(slot -> {
            ...
            actionButtons.add(DialogButtons.action(label).width(30).action(DialogActions.customClick(ACTION_SLOT,
                NightNbtHolder.builder().put(JSON_SLOT, slot).build())).build());
        });
