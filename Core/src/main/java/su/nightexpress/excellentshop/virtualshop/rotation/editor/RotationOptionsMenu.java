...
public class RotationOptionsMenu extends LinkedMenu<ShopPlugin, Rotation> {
    ...
    public RotationOptionsMenu(@NonNull ShopPlugin plugin, @NonNull VirtualShopModule module) {
        ...
        this.addItem(NightItem.asCustomHead(SKULL_DELETE), VirtualLocales.ROTATION_DELETE, 8, (viewer, event,
                                                                                               rotation) -> {
        ...
        this.addItem(NightItem.asCustomHead(SKULL_RESET), VirtualLocales.ROTATION_RESET, 0, (viewer, event,
                                                                                             rotation) -> {
            this.runNextTick(() -> plugin.getShopManager().openConfirmation(viewer.getPlayer(), Confirmation.builder()
                .onAccept((viewer1, event1) -> {
        ...
        this.addItem(Material.ITEM_FRAME, VirtualLocales.ROTATION_EDIT_ICON, 4, (viewer, event, rotation) -> {
            ...
            item.inherit(this.getLink(viewer).getIcon()).localized(VirtualLocales.ROTATION_EDIT_ICON).setHideComponents(
                true);
        }).build());
        ...
        this.addItem(NightItem.asCustomHead(SKULL_CLOCK), VirtualLocales.ROTATION_EDIT_INTERVAL, 21, (viewer, event,
                                                                                                      rotation) -> {
        ...
        }, ItemOptions.builder().setVisibilityPolicy(viewer -> this.getLink(viewer)
            .getRotationType() == RotationType.INTERVAL).build());

        this.addItem(NightItem.asCustomHead(SKULL_CLOCK), VirtualLocales.ROTATION_EDIT_TIMES, 21, (viewer, event,
                                                                                                   rotation) -> {
            this.runNextTick(() -> module.openRotationTimes(viewer.getPlayer(), rotation));
        }, ItemOptions.builder().setVisibilityPolicy(viewer -> this.getLink(viewer)
            .getRotationType() == RotationType.FIXED).build());

        this.addItem(NightItem.asCustomHead(SKULL_SLOTS), VirtualLocales.ROTATION_EDIT_SLOTS, 23, (viewer, event,
                                                                                                   rotation) -> {
            this.runNextTick(() -> module.openRotationSlots(viewer.getPlayer(), rotation));
        });

        this.addItem(NightItem.asCustomHead(SKULL_ITEMS), VirtualLocales.ROTATION_EDIT_PRODUCTS, 25, (viewer, event,
                                                                                                      rotation) -> {
