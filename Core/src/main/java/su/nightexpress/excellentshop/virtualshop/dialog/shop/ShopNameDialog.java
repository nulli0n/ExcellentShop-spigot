...
public class ShopNameDialog extends Dialog<VirtualShop> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.ShopName.Title").text(title("Shop",
        "Name"));
    private static final DialogElementLocale BODY  = VirtualLang.builder("Dialog.ShopName.Body").dialogElement(
        ...
        "",
        "Feel free to use the " + SOFT_YELLOW.wrap(SHOP_NAME) +
            " placeholder in configuration files to \"insert\" shop name in GUIs and messages.",
        ""
    );

    private static final DialogElementLocale INPUT_NAME = VirtualLang.builder("Dialog.ShopName.Input.ShopName")
        .dialogElement("Name");
