package su.nightexpress.excellentshop.virtualshop.dialog.shop;

import org.bukkit.entity.Player;
import org.jspecify.annotations.NonNull;

import su.nightexpress.excellentshop.ShopPlaceholders;
import su.nightexpress.excellentshop.virtualshop.VSFiles;
...
public class ShopLayoutsDialog extends Dialog<VirtualShop> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.ShopLayouts.Title").text(title("Shop",
        "Page Layouts"));
    ...
    private static final TextLocale INPUT_PAGE    = VirtualLang.builder("Dialog.ShopLayouts.Input.Page").text(
        SOFT_YELLOW.wrap("Page #" + GENERIC_PAGE));
    private static final TextLocale INPUT_DEFAULT = VirtualLang.builder("Dialog.ShopLayouts.Input.Default").text(
        SOFT_YELLOW.wrap("Global Layout"));
    ...
    private static final ButtonLocale BUTTON_MODE_PAGINATED = VirtualLang.builder(
        "Dialog.ShopLayouts.Button.ModePaginated")
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull VirtualShop shop) {
        ...
        for (int page = startPage; page < maxPages; page++) {
            ...
            inputs.add(DialogInputs.singleOption(String.valueOf(page), inputLocale.text().replace(
                ShopPlaceholders.GENERIC_PAGE, String.valueOf(page)), entries).build());
        }

        return Dialogs.create(builder -> builder
            .base(DialogBases.builder(TITLE)
                .body(DialogBodies.plainMessage(BODY.replace(str -> str.replace(ShopPlaceholders.GENERIC_PATH, module
                    .getPath().resolve(VSFiles.DIR_LAYOUTS).toString().replace(File.separatorChar, '/')))))
                .inputs(inputs)
            ...
            .type(
                DialogTypes.multiAction(
                    DialogButtons.action(buttonLocale).action(DialogActions.customClick(ACTION_MODE, NightNbtHolder
                        .builder().put(JSON_MODE, !isPerPage).build())).build(),
                    DialogButtons.apply()
                )
