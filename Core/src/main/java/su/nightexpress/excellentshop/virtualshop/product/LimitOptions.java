...
public class LimitOptions implements Writeable {
    ...
    @NonNull
    public static LimitOptions read(@NonNull FileConfig config, @NonNull String path) {
        ...
        config.set(path + ".Enabled", config.getInt(path + ".BuyAmount") >= 0 || config.getInt(path +
            ".SellAmount") >= 0);
    ...
    public boolean isUnlimited(@NonNull TradeType type) {
    ...
    public boolean hasLimit(@NonNull TradeType type) {
        return switch (type) {
            case BUY -> this.hasBuyLimit();
            case SELL -> this.hasSellLimit();
        };
    }
    ...
    public int getLimit(@NonNull TradeType type) {
        return switch (type) {
    ...
    public void setLimit(@NonNull TradeType type, int amount) {
        switch (type) {
