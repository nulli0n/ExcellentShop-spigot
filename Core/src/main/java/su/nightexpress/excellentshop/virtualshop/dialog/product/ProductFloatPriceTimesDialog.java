...
public class ProductFloatPriceTimesDialog extends Dialog<VirtualProduct> {
    ...
    private static final TextLocale TITLE = VirtualLang.builder("Dialog.ProductPrice.FloatTimes.Title").text(title(
        "Float Price", "Refresh Times"));

    private static final DialogElementLocale BODY_INTERVAL = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Body.Interval").dialogElement(
            400,
    ...
    private static final DialogElementLocale BODY_SCHEDULED = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Body.Scheduled").dialogElement(
            400,
            "In " + SOFT_YELLOW.wrap("Scheduled") + " mode, the item’s price is updated strictly at the specified " +
                SOFT_YELLOW.wrap("hours") + " on selected " + SOFT_YELLOW.wrap("days") + " of the week.",
            "",
            SOFT_YELLOW.wrap("→") + " To set the " + SOFT_YELLOW.wrap("hours") + ", enter the desired times in the " +
                SOFT_YELLOW.wrap("Times") + " field " + GRAY.wrap("(one time per line)") + ".",
            "",
            SOFT_YELLOW.wrap("→") + " To choose the " + SOFT_YELLOW.wrap("days") +
                ", click the weekday buttons. The selected days are highlighted in " + GREEN.wrap("green") + "."
        );

    private static final DialogElementLocale INPUT_INTERVAL = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Input.Interval")
        .dialogElement(120, "Interval " + SOFT_YELLOW.wrap("(in seconds)"));

    private static final DialogElementLocale INPUT_TIMES = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Input.Times")
        .dialogElement(150, "Times " + SOFT_YELLOW.wrap("(in 24-hour HH:MM format)"));

    private static final ButtonLocale BUTTON_DAY_ACTIVE = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Button.DayActive")
        .button(GREEN.wrap("[X]") + " " + GENERIC_NAME, 100);

    private static final ButtonLocale BUTTON_DAY_INACTIVE = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Button.DayInactive")
        .button(GRAY.wrap("[ ] " + GENERIC_NAME), 100);

    public static final ButtonLocale BUTTON_MODE_INTERVAL = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Button.ModeInterval")
        .button(SOFT_YELLOW.wrap("→") + " Switch to " + SOFT_YELLOW.wrap("Interval") + " mode");

    public static final ButtonLocale BUTTON_MODE_TIMES = VirtualLang.builder(
        "Dialog.ProductPrice.FloatTimes.Button.ModeScheduled")
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull VirtualProduct product) {
        if (!(product.getPricing() instanceof FloatPricing pricing))
            throw new IllegalArgumentException("Product does not have a Floating Price");
        ...
        if (refreshType == RefreshType.INTERVAL) {
            inputs.add(DialogInputs.text(JSON_INTERVAL, INPUT_INTERVAL).initial(String.valueOf(pricing
                .getRefreshInterval())).build());
            ...
            for (DayOfWeek day : DayOfWeek.values()) {
                ButtonLocale locale = (pricing.hasDay(day) ? BUTTON_DAY_ACTIVE : BUTTON_DAY_INACTIVE).replace(s -> s
                    .replace(GENERIC_NAME, Lang.DAYS.getLocalized(day)));

                buttons.add(DialogButtons.action(locale).action(DialogActions.customClick(ACTION_DAY, NightNbtHolder
                    .builder().put(JSON_DAY, day.name()).build())).build());
            }

            buttons.add(DialogButtons.action(BUTTON_MODE_INTERVAL).action(DialogActions.customClick(ACTION_MODE))
                .build());
        }
