...
public class ShopAliasesDialog extends Dialog<VirtualShop> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.ShopAliases.Title").text(title("Shop",
        "Aliases"));
    private static final DialogElementLocale BODY  = VirtualLang.builder("Dialog.ShopAliases.Body").dialogElement(
        ...
        "These aliases will be registered as " + SOFT_YELLOW.wrap("server commands") +
            ", and running one of them will open this shop.",
        "",
        SOFT_ORANGE.wrap("⚠") + " Players must have the " + SOFT_ORANGE.wrap(GENERIC_PERMISSION) +
            " permission to access these commands.",
        "",
        SOFT_RED.wrap("→") + " Enter " + SOFT_RED.wrap("one") + " alias " + SOFT_RED.wrap("per line") + "."
    );

    private static final DialogElementLocale INPUT_ALIASES = VirtualLang.builder("Dialog.ShopAliases.Input.Text")
        .dialogElement("Aliases");
    private static final TextLocale          INPUT_RELOAD  = VirtualLang.builder("Dialog.ShopAliases.Input.Reload")
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull VirtualShop shop) {
        return Dialogs.create(builder -> builder
            .base(DialogBases.builder(TITLE)
                .body(DialogBodies.plainMessage(BODY.replace(str -> str.replace(ShopPlaceholders.GENERIC_PERMISSION,
                    VirtualPerms.COMMAND_SHOP.getName()))))
                .inputs(
    ...
    private void setShopAliases(@NonNull VirtualShop shop, @NonNull String[] aliases, boolean reload) {
        shop.setAliases(Arrays.stream(aliases).map(s -> s.startsWith("/") && s.length() >= 2 ? s.substring(1) : s)
