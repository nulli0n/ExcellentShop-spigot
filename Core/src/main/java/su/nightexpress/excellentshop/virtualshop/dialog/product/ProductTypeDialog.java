...
public class ProductTypeDialog extends Dialog<VirtualProduct> {
    ...
    private static final TextLocale          TITLE = VirtualLang.builder("Dialog.Product.ContentType.Title").text(title(
        "Product", "Type"));
    private static final DialogElementLocale BODY  = VirtualLang.builder("Dialog.Product.ContentType.Body")
        .dialogElement(
            400,
            "Select the desired product type.",
            "",
            TagWrappers.SOFT_YELLOW.wrap("→") + " Item " + TagWrappers.GRAY.wrap("(Physical)") +
                " - Standard buying and selling of a specific item.",
            TagWrappers.SOFT_YELLOW.wrap("→") + " Command " + TagWrappers.GRAY.wrap("(Virtual)") +
                " - Executes specified commands upon purchase. Cannot be sold.",
            "",
            TagWrappers.SOFT_RED.wrap("All current product settings dependent on the previous type will be " +
                TagWrappers.UNDERLINED.wrap("OVERWRITTEN") + "!")
        );
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull VirtualProduct product) {
        ...
        for (ContentType type : ContentType.values()) {
            ...
            ButtonLocale locale = BUTTON_TYPE.replace(str -> str.replace(ShopPlaceholders.GENERIC_TYPE,
                Lang.CONTENT_TYPE.getLocalized(type)));
            ...
            builder.handleResponse(actionId, (viewer, identified, nbtHolder) -> {
                ProductContent content = ContentTypes.create(type, product.getEffectivePreview(),
                    this.module::isItemProviderAllowed);
