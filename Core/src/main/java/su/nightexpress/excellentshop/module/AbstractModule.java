...
import su.nightexpress.excellentshop.exception.ModuleLoadException;
...
public abstract class AbstractModule extends AbstractManager<ShopPlugin> implements Module {
    ...
    protected final void onLoad() throws ModuleLoadException {
        ...
        this.info("Enabled currencies: " + this.getEnabledCurrencies().stream().map(Currency::getInternalId).collect(
            Collectors.joining(", ")));
    ...
    public boolean isEnabledCurrency(@NonNull String id) {
        return this.definition.getDefaultCurrency().equalsIgnoreCase(id) || this.definition.getEnabledCurrencies()
            .contains(id) || this.definition.getEnabledCurrencies().contains(ShopPlaceholders.WILDCARD);
    }
    ...
    public Set<Currency> getEnabledCurrencies() {
        if (this.definition.getEnabledCurrencies().contains(ShopPlaceholders.WILDCARD)) return EconomyBridge.api()
            .getCurrencies();
    ...
    public Set<Currency> getAvailableCurrencies(@NonNull Player player) {
        ...
        return currencies.stream().filter(currency -> this.canUseCurrency(player, currency)).collect(Collectors
            .toSet());
    }
