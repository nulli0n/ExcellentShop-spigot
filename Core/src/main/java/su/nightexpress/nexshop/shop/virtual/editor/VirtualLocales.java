...
public class VirtualLocales extends su.nexmedia.engine.api.editor.EditorLocales {
    ...
    public static final EditorLocale PRODUCT_RANKS_AND_PERMS = builder(PREFIX + "Product.Ranks_Permissions")
        .name("Ranks & Permissions")
        .text("Here you configure:")
        .text("- List of ranks (permission groups) which", "    can access this product.")
        .text("- List of permissions required", "    to access this product.")
        .text("(leave list(s) empty to disable)")
        .emptyLine()
        .text(YELLOW + BOLD + "Allowed Ranks:")
        .text(Placeholders.PRODUCT_ALLOWED_RANKS)
        .emptyLine()
        .text(YELLOW + BOLD + "Required Permissions:")
        .text(Placeholders.PRODUCT_REQUIRED_PERMISSIONS)
        .emptyLine()
        .text("(" + WHITE + LMB + GRAY + " to add rank)")
        .text("(" + WHITE + RMB + GRAY + " to remove ranks)")
        .text("(" + WHITE + SHIFT_LMB + GRAY + " to add permission)")
        .text("(" + WHITE + SHIFT_RMB + GRAY + " to remove permissions)")
        .build();
    ...
    public static final EditorLocale PRODUCT_PRICE_FLOAT_REFRESH = builder(PREFIX + "Product.Price.Float.Refresh")
        ...
        .text("Until that, previously generated price will be used.")
        .emptyLine()
        .currentHeader()
        .current("Round Decimals", Placeholders.PRODUCT_PRICER_FLOAT_ROUND_DECIMALS)
        .current("Days", Placeholders.PRODUCT_PRICER_FLOAT_REFRESH_DAYS)
        .current("Times", Placeholders.PRODUCT_PRICER_FLOAT_REFRESH_TIMES)
        .emptyLine()
        .click(DROP_KEY, "toggle Decimals")
