...
public record ECompletedTransaction(
    ...
    @NonNull
    public static ECompletedTransaction create(@NonNull EPreparedTransaction origin, @NonNull ETransactionResult result,
                                               @NonNull BalanceHolder worth) {
        return new ECompletedTransaction(
            origin.getPlayer(), origin.getType(), List.copyOf(origin.getItemsList()), List.copyOf(origin
                .getLooseItems()), origin.getUserInventory(), worth, result, origin.isSilent()
        );
    }
    ...
    public int countTotalAmount() {
        return this.items.stream().mapToInt(quantified -> UnitUtils.unitsToAmount(quantified.product(), quantified
            .units())).sum();
    }
